...
scaled_sub_df_low = scaled_sub_df_normal.copy()
for i in scaled_sub_df_low.index:
    scaled_sub_df_low.loc[i, 'view'] = max(scaled_sub_df_low.loc[i, 'view'] - 15, 0)
resample_df_low = scaled_sub_df_low.resample('20min').mean()

scaled_sub_df_high = scaled_sub_df_normal.copy()
for i in scaled_sub_df_high.index:
    scaled_sub_df_high.loc[i, 'view'] = max(scaled_sub_df_high.loc[i, 'view'] + 15, 0)
resample_df_high = scaled_sub_df_high.resample('20min').mean()

used_scaled_sub_df = scaled_sub_df_normal
...
plt.plot(list(range(len(resample_df_normal.index))), resample_df_normal['view'], label='nomarl Request(request/s)')
