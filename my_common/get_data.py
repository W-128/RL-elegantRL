...
def get_arrive_time_request_dic_from_request_record(arrive_time_index):
    ...
    new_arrive_request_in_dic = []
    # curPath = os.path.abspath(os.path.dirname(__file__))
    filename = '../server/request_record.csv'
    data = pd.read_csv(filename, header=0)
    ...
    for i in range(0, len(data)):
        ...
        new_arrive_request_in_dic.append(request_in_dic)
        new_arrive_request_in_dic = sorted(new_arrive_request_in_dic, key=lambda i: i[1])

    base_time = new_arrive_request_in_dic[0][1].replace(microsecond=0)
    for i in range(0, len(new_arrive_request_in_dic)):
        delta_second = int(request_in_dic[1].timestamp() - base_time.timestamp())
        request_in_dic_second = list(new_arrive_request_in_dic)
